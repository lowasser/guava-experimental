...
abstract class AbstractConcurrentMultiset<E> extends AbstractMultiset<E> implements Serializable {
  /*
   * The AbstractConcurrentMultiset atomic operations are implemented primarily in terms of
  ...
  private static class FieldSettersHolder {
    static final FieldSetter<AbstractConcurrentMultiset> COUNT_MAP_FIELD_SETTER =
        Serialization.getFieldSetter(AbstractConcurrentMultiset.class, "countMap");
  }
  
  private transient final ConcurrentMap<E, AtomicInteger> countMap;
  
  AbstractConcurrentMultiset(ConcurrentMap<E, AtomicInteger> countMap) {
    checkArgument(countMap.isEmpty());
    this.countMap = countMap;
  }
  
  ConcurrentMap<E, AtomicInteger> backingMap() {
    return countMap;
  }
  ...
  private List<E> snapshot() {
    ...
    for (Multiset.Entry<E> entry : entrySet()) {
      list.addAll(Collections.nCopies(entry.getCount(), entry.getElement()));
    }
    return list;
  }
  ...
  @Override Iterator<Entry<E>> entryIterator() {
    ...
    return new ForwardingIterator<Entry<E>>() {
      ...
      @Override public void remove() {
        ...
        AbstractConcurrentMultiset.this.setCount(last.getElement(), 0);
        last = null;
      }
    };
  }
  ...
  private class EntrySet extends AbstractMultiset<E>.EntrySet {
    @Override Multiset<E> multiset() {
      return AbstractConcurrentMultiset.this;
    }
